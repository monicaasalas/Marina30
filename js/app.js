...
function render(){const id=qs(); if(!id) return home(); const m=missions.find(x=>x.id===id); if(!m) return home(); if(m.type==='quiz')return quiz(id); if(id==='musica')return music(); if(id==='letras')return partyLetters(); if(id==='dibujo')return drawing(); if(id==='inutil')return useless(); if(id==='diferencias')return differences(); if(id==='quien')return identityGame();}
